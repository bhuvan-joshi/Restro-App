...
const { importProductsWithImagesFromExcel } = require('../utils/excelImageImport');
...
const fileFilter = (req, file, cb) => {
  // Allow Excel files for import endpoint, images elsewhere
  const isExcel = file.originalname.match(/\.(xlsx|xls)$/i);
  const isImage = file.originalname.match(/\.(jpg|jpeg|png|gif|webp)$/i);

  // If the upload field is 'excelFile' (used in import), allow Excel files
  if (file.fieldname === 'excelFile') {
    if (!isExcel) {
      return cb(new Error('Only Excel files are allowed for import!'), false);
    }
    return cb(null, true);
  }

  // Otherwise, allow only images (including webp)
  if (!isImage) {
    // Set a custom property on req to indicate the error
    req.uploadError = 'Only image files (jpg, jpeg, png, gif, webp) are allowed!';
...
router.post(
  ...
  function (req, res, next) {
    uploadImage.single('image')(req, res, function (err) {
      if (err) {
        return res.status(400).json({ message: err.message || 'Image upload failed. Please upload a valid image file (jpg, jpeg, png, gif, webp) under 5MB.' });
      }
      if (req.uploadError) {
...
router.post(
  ...
  async (req, res) => {
    ...
    if (fileExt !== '.xlsx') {
      ...
      return res.status(400).json({ message: 'Only .xlsx Excel files are allowed for image import.' });
    }

    try {
      // Process the Excel file using our JavaScript utility
      const db = req.app.locals.db;
      const result = await importProductsWithImagesFromExcel(req.file.path, db, req.user.id);
