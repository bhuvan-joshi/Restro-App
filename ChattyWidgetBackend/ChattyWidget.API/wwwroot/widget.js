(function() {
  ...
  cssLink.href = 'https://localhost:7216/api/widgetresources/css'; // Change to your production URL
  ...
  const loadWidget = async () => {
    try {
      ...
      const response = await fetch(`https://localhost:7216/api/widgetsettings/public/${widgetId}`);
      ...
      button.addEventListener('click', async () => {
        ...
        if (!session) {
          try {
            ...
            const sessionResponse = await fetch('https://localhost:7216/api/chat/public/sessions', {
              method: 'POST',
              headers: {
      ...
      form.addEventListener('submit', async (e) => {
        ...
        try {
          const messageResponse = await fetch('https://localhost:7216/api/chat/public/messages', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
          ...
          const updatedSessionResponse = await fetch(`https://localhost:7216/api/chat/public/sessions/${session.id}`);
      ...
      function startPolling() {
        ...
        pollInterval = setInterval(async () => {
          ...
          try {
            const response = await fetch(`https://localhost:7216/api/chat/public/sessions/${session.id}`);
