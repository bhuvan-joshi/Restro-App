...
namespace ChattyWidget.Core.Services
{
    public class AnthropicLlmService : ILlmService
    {
            ...
            string modelId)
        {
            try
            {
                // Validate model ID and provider
                if (!_modelProviderMap.TryGetValue(modelId, out var provider) || provider != "Anthropic")
                {
                    throw new ArgumentException($"Model {modelId} is not supported by Anthropic provider");
                }
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    // Claude doesn't return confidence scores, so we use a fixed value
                ...
                else
                {
                    ...
                    _logger.LogError($"Error from Anthropic API: {response.StatusCode} - {errorContent}");
                    throw new Exception($"Error from LLM provider: {response.StatusCode}");
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"Error generating response with Anthropic");
                throw;
            }
        }
        ...
        {
            // Anthropic service doesn't provide Ollama models, so return an empty list
            ...
            Func<Task> onComplete)
        {
            ...
            // A proper streaming implementation for Anthropic would use their streaming API
        ...
        private string FormatDocumentsAsContext(List<string> documentContents, List<string> documentTitles)
        {
            ...
            // Using rough token estimates for Claude models (1 token ~= 4 chars)
            int maxContextLength = 4000; // Maximum chars per document - Claude has larger context
            int maxTotalContextLength = 16000; // Maximum total context size
        ...
        {
            // Enhanced system prompt for Claude with better document understanding instructions
...
Claude is especially good at carefully analyzing document content. Leverage this capability by thoroughly examining all provided documents before answering any questions. Your goal is to be accurate, helpful, and thorough while strictly limiting your responses to what can be found in the provided documents.";
        }
